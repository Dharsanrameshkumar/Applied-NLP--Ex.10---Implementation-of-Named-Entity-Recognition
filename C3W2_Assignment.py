...
def NER(len_tags, vocab_size, embedding_dim = 50):
    ...
    # The input_dim is vocab_size + 1 to account for mask_zero=True
...
def masked_loss(y_true, y_pred):
    ...
    Parameters:
    y_true (tensor): True labels, where -1 indicates padding to be ignored.
    y_pred (tensor): Predicted logits.
    
    Returns:
    loss (tensor): Calculated loss, excluding padded elements.
    """
    
    # Create a mask to ignore padding values (where y_true == -1)
    # The mask is True for non-padding elements, False for padding.
    mask = tf.math.not_equal(y_true, -1)
    
    # Initialize the loss function. 
    # from_logits=True means y_pred are raw scores (logits), not probabilities.
    loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
    
    # Create a "clean" y_true, replacing -1 with 0 (a valid class index)
    # This is necessary because the loss function cannot handle a label of -1.
    # The mask ensures these positions are ignored anyway.
    y_true_clean = tf.where(mask, y_true, 0)
    
    # Compute the loss, using the mask as sample_weight to ignore padding elements.
    # Elements where mask is False (padding) contribute 0 to the loss.
    loss = loss_fn(y_true_clean, y_pred, sample_weight=mask)
...
def masked_accuracy(y_true, y_pred):
    ...
    ### START CODE HERE ###
    # Convert true labels to float for safe math operations
    y_true = tf.cast(y_true, tf.float32)

    # Create mask to ignore padding tokens (which are -1)
    mask = tf.not_equal(y_true, -1)
    mask = tf.cast(mask, tf.float32)

    # Get predicted class IDs from logits (along last dimension)
    ...
    # Apply the mask — ignore padding tokens
...
def predict(sentence, model, sentence_vectorizer, tag_map):
    ...
    # Expand dimension to match model input (batch size = 1)
    sentence_vectorized = tf.expand_dims(sentence_vectorized, axis=0)

    # Get model predictions (logits)
    output = model(sentence_vectorized)

    # Get the predicted class IDs per token
    outputs = np.argmax(output, axis=-1)

    # Adjust dimensions (from [[1, 2, 3, ...]] to [1, 2, 3, ...])
    outputs = outputs[0]

    # Map predicted IDs to tag names using tag_map
